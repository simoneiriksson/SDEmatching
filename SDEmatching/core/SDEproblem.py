...
class SDEproblem(torch.nn.Module):
    ...
    def ELBO(self, data_batch, data_mask):
        this_batch_size = len(data_batch)
        # finding prior loss
        t0_batch = torch.ones(this_batch_size, device=self.device) * self.t_start
        z_0, z_0_logprob = self.marginal.sample_and_log_prob(t0_batch, data_batch, data_mask, 1)  # get sample and corresponding logprob from marginal distribution at t=t_start 
        ...
        cond = self.condition_mapper(t_diff, data_batch, data_mask) # shape (batch, state_dim * 2)
        z_diff, z_diff_logprob = self.marginal.forward_and_log_prob(eps1, t_diff, data_batch, data_mask) # shape (batch, state_dim), (batch)
        ...
        fd1 = self.marginal.SDEforwarddrift(z_diff, t_diff, data_batch, data_mask)  # shape (batch, state_dim)
        ...
        valid_counts = (~data_mask).sum(dim=1)  # (B,) — number of real timesteps per series

        # Sample a random index in [0, valid_count) for each series
        rand_fractions = torch.rand(this_batch_size, device=self.device)
        rec_ns = (rand_fractions * valid_counts.float()).long()  # (B,) — all within valid range

        batch_indices = torch.arange(this_batch_size, device=self.device)
        data_batch_slice = data_batch[batch_indices, rec_ns]
        data_batch_slice = data_batch[batch_indices,rec_ns]
        z_rec, z_rec_logprob = self.marginal.sample_and_log_prob(data_batch_slice[:,0], data_batch, data_mask, 1)
